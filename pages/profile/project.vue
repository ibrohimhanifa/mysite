        ...
        <div class="container px-5 py-24 mx-auto">
          ...
          <div class="flex flex-wrap sm:-m-4 -mx-4 -mb-10 -mt-4">
            <div class="p-4 md:w-1/3 md:mb-0 mb-6 flex flex-col justify-center items-center max-w-sm mx-auto" v-for="project in projects" :key="project">
              ...
              <div class=" w-60 bg-white -mt-10 shadow-lg rounded-lg overflow-hidden p-5">
                <div class="header-content inline-flex ">
                  <div class="category-badge flex-1  h-4 w-4 m rounded-full m-1 bg-purple-100">
                    <div class="h-2 w-2 rounded-full m-1 bg-purple-500 " ></div>
                  </div>
                  <div class="category-title flex-4 text-sm">{{ project.name }} ({{ project.start }} - {{ project.end }})</div>
                </div>
                <div class="title-post font-bold">{{ project.role }}</div>
                <div class="summary-post text-base">{{ project.desc }}
                  <button class="bg-blue-100 text-blue-500 mt-4 block rounded p-2 text-sm "><span class="">{{project.categ}}</span></button>
                </div>
              </div>
            </div>
          </div>
